...
### K-4 축·감사 스코프
- 이벤트 축 = **기존 파티션 축(accountId) 유지**. **`ownerId`(값=CustomerId)는 IS-1 선반영 6종 대상 이벤트에만** 추가. 테넌트 축은 형태만(IS-7 — 필드 예약 안 함).
- 감사 AD-8 스코프 칸 = ★ **판정에 실제 쓰인 스코프 값**(운영자 = 조직 축 값 · 고객 = 소유) — 재현 가능성 기준, 혼재 해소.
