...
> 이 문서를 다 읽으면 **20개 유스케이스가 어떤 액터·경계로 갈리는지, 그 계약이 어떤 엔드포인트·이벤트로 전수 등재되는지, 그리고 오류·멱등·명명·축이 어떤 공통 규약(K-1~4)을 따르는지**를 어디서 찾을지 알 수 있어야 한다.\
> 이 문서는 색인이다 — 각 UC의 본문이 정본이고, 여기서는 대장(유스케이스·엔드포인트·이벤트·도출표)과 계약 공통 규약만 모은다.

## 1. 유스케이스 대장

아래 표는 20개 유스케이스를 번호·액터·트랜잭션 경계·상태로 늘어놓은 대장이다.
...
아래 표는 계약이 노출하는 인터페이스를 UC·주체·L7 대상으로 전수 등재한 것이다 — BR-58 존재 비노출 시험의 입력이다.
...
아래 표는 각 이벤트를 UC·행위자 판정(AD-7)·축 필드·S-9 스키마 버전으로 전수 등재한 것이다.
...
아래 표는 모든 조작·운영자 규칙·SM 전이를 어느 UC(또는 운영 절차·내부)로 배정했는지 전수로 판정한 것이다 — 빈 칸이 곧 결함이다.
...
아래 네 묶음(K-1 오류·멱등 · K-2 명명 · K-3 스키마 버전 · K-4 축·감사 스코프)은 모든 UC 계약이 공통으로 따르는 규약의 정본이다.
...
| 버전 | 일자 | 내용 |
|---|---|---|
| v0.15 | 2026-09-16 | **가독성 재작성**(작성법 분리/복원/이연 — 내용 무변). 상단 정향 문단 신설 + §1~§5 절 lead-in 보강만 적용 · 대장 표(유스케이스·엔드포인트·이벤트·도출표·K-1~4)는 셀 1줄 그대로 유지. 코드·규칙 ID·판정 전부 보존 |
