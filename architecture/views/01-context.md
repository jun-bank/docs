...
| **외부 입금원** | 타행 이체 | ❌ **모의** |
| **운영자** | 보류·차단 해제·정정·수동 실행 | 액터. ★ **권한 3단계 확정**(조회/담당자/책임자 — BR-55, 2026-08-05. G-2 닫힘) |
...
| 운영자 | 우리 | 관리 조작 (단계 = BR-55 · 책임자 승인 **5조작** = BR-56) | 동기 | 권한 밖 → 거절 + **시도 기록**(BR-57) |
| ★ 카드소지자 | 우리 | **조회** (거래·잔액 — BR-31·58) | 동기 | 타인 식별자 → 거절. ★ **응답이 "없음"과 구별되지 않는다**(L7). 그림에는 있었는데 이 표에 없던 행이다(2026-08-05) |
...
| 버전 | 일자 | 내용 |
|---|---|---|
| v0.2 | 2026-08-05 | **멀티테넌시 설계 변경 (B-5)** — §3에 **카드소지자 조회 행 추가**(그림 `CH→SYS`에는 있는데 표에 없던 DC-004형 "한쪽만 적음") · 운영자 행에 권한 단계·승인·실패 시 기록 반영(G-2 닫힘) |
