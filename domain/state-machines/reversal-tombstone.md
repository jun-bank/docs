...
| 만료됨 | `EXPIRED` | 보관 기간이 지나도록 승인이 오지 않았다 | **✓** | TTL 경과 |
...
| **RF2** | `EXPIRED` | `consume` | 보관 기간이 지났다 | 오래된 취소 의사로 **한참 뒤의 새 승인**을 죽인다 | `TOMBSTONE_EXPIRED` |
| **RF3** | `CONSUMED`·`EXPIRED` | `record` (**같은 키**) | INV-1 위반 | 같은 상관 식별자에 예약이 둘 → 중복 무효화 | **오류가 아니라 무시** (◎) — 중복 망취소는 정상이다 (BR-13) |
...
|---|---|---|---|
| **`PENDING`** | ◎ (RF3) | **O** (R2) | **O** (R3) |
| **`CONSUMED`** | ◎ (RF3) | X (RF1) | X (RF4) |
| **`EXPIRED`** | ◎ (RF3) | X (RF2) | ◎ |

**O 2 · X 3 · ◎ 4 = 9칸**

> **`record`가 전 상태에서 `◎`인 것이 핵심이다.** 중복 망취소는 **정상 경로**이므로(BR-13) 오류로 만들면 안 된다. 다만 새 예약을 만들지도 않는다 — 이미 있는 것이 결말을 정한다.
