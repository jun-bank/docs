...
> 이 문서를 다 읽으면 **왜 이 애그리게이트가 존재하는지(망취소가 원 승인보다 먼저 올 수 있다), 그리고 "유효한 예약"을 왜 존재가 아니라 시각으로 판정해야 하는지**를 설명할 수 있어야 한다.

이 애그리게이트는 두 장군 문제의 실물이다.\
망취소가 원 승인보다 먼저 도착할 수 있다는 사실 하나 때문에 존재한다(BR-22).

---

## 0. "유효한 예약"의 정의 ★

유효성은 존재만으로 정해지지 않는다 — 시각이 함께 든다.\
아래가 그 정의다.
...
**만료 시각은 지났는데 정리 배치가 아직 안 돈 구간**이 반드시 존재한다.\
그 레코드는 **살아 있지만 써서는 안 된다.**

이 정의를 안 쓰면 승인 경로가 어느 쪽으로도 못 가는 교착이 생긴다.\
아래 표가 그 두 조건의 충돌이다.
...
> **배치를 믿지 않는다.**\
> `consume()`이 시각을 직접 판정하는 것과 같은 이유로, **승인 경로도 "존재"가 아니라 "유효"로 판정**해야 한다.\
> 그러지 않으면 RTS2가 말한 TTL 경계 위험이 **경합이 아니라 배치 지연만으로도** 발생한다.
...
> **만료가 정상 결말일 수 있다.**\
> 승인 전문이 아예 우리에게 도달하지 않았다면 무효화할 대상이 없다 — 예약은 쓰이지 않고 만료된다.\
> **이것은 실패가 아니다.**
>
> ⚠️ **만료를 `EXPIRED` 상태로 두지 않고 `ABSENT`로 되돌린다.**\
> 애그리게이트 `expire()`의 사후조건이 **레코드 삭제**이기 때문이다.\
> 그래서 만료 뒤 같은 상관 식별자로 망취소가 오면 **새 예약이 생성된다** — 그것이 맞다, 새 취소 의사이므로.\
> 존재하는 채로 두고 `record`를 무시하면 오래된 취소 의사가 관계없는 새 거래를 계속 막는다.

---

## 2. 전이도

생성·소비·만료·purge의 전이를 한 그림으로 보인다.
...
네 전이의 조건·부수 효과·근거를 정리한다.\
R1의 유일성 키와 소비가 승인 성립과 한 트랜잭션인 이유는 표 뒤에 따로 둔다.

| # | 현재 | 전이 | 다음 | 조건 | 부수 효과 | 근거 |
|---|---|---|---|---|---|---|
| R1 | `ABSENT` | `record` | `PENDING` | ★ **같은 기관의 같은 `correlationId`** 예약 없음 — `(발신 기관, correlationId)` 유일(INV-1 · 2026-08-06 **R-06**) | `expiresAt` 설정 | BR-22 |
...
R1의 유일성 키를 풀어 쓰면 이렇다.\
상관 식별자는 매입사가 채번하며, **기관 = ACL이 인증한 발신 기관**이다 — X-6 기관 코드 일치를 검증한다.

### 소비는 승인 성립과 같은 트랜잭션이다

소비와 승인 생성을 한 트랜잭션에 묶는다.\
아래 그림이 그 경계다.
...
**나누면 그 사이에 예약이 소비됐는데 승인이 없는 순간이 생긴다.**\
그때 같은 상관 식별자로 승인이 또 오면 **예약이 이미 소비되어 통과**한다 — 취소되었어야 할 홀딩이 되살아난다.
...
> **RF1이 이 문서에서 가장 위험하다.**\
> 예약이 반복 소비되면 **정상 거래가 죽는다.**\
> 망취소는 응답 유실 때 오는 것이라 매입사가 같은 상관 식별자로 재시도할 수 있고, 그중 하나는 정상 승인일 수 있다.
...
칸을 세면 **O 3 · O/X 1 · O/◎ 1 · X 3 · ◎ 1 · - 3 = 12칸**이다.

> **중복 망취소는 정상 경로다**(BR-13) — 오류로 만들면 안 된다.\
> 다만 **유효한** 예약이 이미 있으면 새로 만들지 않는다.
>
> ⚠️ **`PENDING`/`record`가 조건부인 이유가 §0이다.**\
> 만료 시각이 지난(무효한) 예약에 새 망취소가 오는데 `◎`로 버리면, **새 취소 의사가 조용히 사라지고** 직후 정리 배치가 삭제하면 뒤늦은 승인을 막을 것이 없어진다.\
> 만료 경과 상태에서는 **새 예약으로 대체**한다.
>
> **`PENDING/consume`이 조건부인 것이 RTS2의 실체다.**\
> 만료 시각은 지났는데 정리 배치가 아직 안 돌았을 때, 그 레코드는 **존재하지만 써서는 안 된다.**\
> `consume()`이 시각을 직접 판정해야 하며(배치를 믿으면 안 된다), 이 판정이 빠지면 오래된 취소 의사가 새 승인을 죽인다.

> **`ABSENT` 행이 초기 상태이자 만료 후 상태다.**\
> 만료는 레코드 삭제이므로 별도 상태가 아니라 여기로 돌아오는 것이며, `record`는 정상적으로 새 예약을 만든다.\
> 이 행을 "존재하며 전부 X"로 적으면 **만료 후 도착한 새 취소 의사를 영영 못 받는다.**
...
동시 상황별 처리를 정리한다.\
마지막 행(소비와 만료 배치의 경합)이 실질 위험이라, 그 경고를 표 뒤에 둔다.
...
> ⚠️ **마지막 행이 실질 위험이다.**\
> TTL 경계에서 만료가 근소하게 이기면 **취소되었어야 할 승인이 홀딩까지 잡고 성립한다.**\
> TTL을 넉넉히 잡는 것 외에 구조적 해법이 없다.

---

## 7. 시간 기반 전이 ★

시간이 일으키는 두 전이 — 만료와 purge다.
...
> **승인 전문의 최대 지연 시간보다 충분히 길어야 한다.**\
> 매입사와 합의된 타임아웃 값이 그 하한이다 — Phase 4 계약 설계에서 확정한다.
...
닫힌 두 건(RTS1·RTS3)과 아직 열린 한 건(RTS2)이다.

| # | 의문 | 영향 |
|---|---|---|
| ~~RTS1~~ | ✅ **닫힘 (2026-09-15 = 애그리게이트 RT2 확정): 만료(보관 기간)까지 유지 후 `purge`.** 즉시 삭제 안 함 — RF5(같은 키 재요청 ◎)·RF4(무효화 추적)가 지키는 방어선이 그 근거다. 보관 기간 값은 미확정(애그리게이트 RT1) | 재전송 처리 |
...
| v0.7 | 2026-09-15 | **RTS1 닫힘 = RT2 확정**(만료까지 유지 후 purge). 애그리게이트 `reversal-tombstone.md` v1.4와 동기. 보관 기간 값은 RT1 이연 |
