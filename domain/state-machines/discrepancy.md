...
> 이 문서를 다 읽으면 **불일치가 어떤 조작을 허용·금지하는지, 그리고 왜 이 애그리게이트에만 종료 상태가 없는지**를 설명할 수 있어야 한다.

> **이 상태 머신에는 종료 상태가 없다.**\
> 처리완료조차 종료가 아니다 — 재발견되면 미처리로 돌아온다(BR-48).\
> 그것이 이 문서의 요점이다.
...
불일치는 세 상태를 갖고, 셋 중 어느 것도 종료가 아니다.\
아래 표가 각 상태의 뜻과 진입 조건이다.
...
아래 표가 불일치의 허용 전이 다섯 개다 — 최초 적재부터 재발견 복귀까지.
...
> **D4에서 `resolution`을 지우지 않는다.**\
> *"지난번엔 이렇게 처리했는데 또 나왔다"* 가 다음 조사의 가장 중요한 단서다.\
> 지우면 운영자가 같은 오판을 반복한다.

> **D3이 `OPEN`에서도 허용된다.**\
> 조사를 거치지 않고 바로 처리해도 된다 — 원인이 명백한 불일치까지 조사 단계를 강제하면 운영자가 형식적으로 두 번 클릭하게 될 뿐이다.\
...
아래 표가 불일치가 막는 전이 일곱 개다 — 이 애그리게이트의 방어선은 금지보다 **조작의 부재**(DF2·DF6)에 더 있다.
...
> **DF2와 DF6은 조작 목록의 부재로 강제된다.**\
> 응답 코드가 아니라 **호출할 메서드가 없다.**\
...
아래 매트릭스가 세 상태 × 세 조작의 판정을 모은 정본이다.
...
> ⚠️ **`recordOrTouch` 칸에 `◎`를 쓰면 안 된다.**\
> `◎`는 "no-op, 최초 결과 반환"인데 이 조작은 **`lastDetectedOn`과 `detectionCount`를 반드시 바꾼다**(BR-48).\
> 상태가 안 변하는 것과 아무 일도 안 일어나는 것은 다르며, `◎`로 적으면 구현자가 그대로 무시해 재발견 추적이 통째로 사라진다.

> **`X`가 3개뿐인 것이 이 표의 특징이다.**\
> 다른 애그리게이트와 달리 이 상태 머신의 방어선은 **금지 전이가 아니라 조작의 부재**(DF2·DF6)에 있다.\
> 고칠 방법이 없으므로 막을 것도 적다.

---

## 6. 동시성

아래 표가 불일치에서 경합이 생기는 네 상황과 그 처리다.
...
시간이 스스로 일으키는 상태 전이는 없다 — 아래 표가 그것을 명시한다.
...
> **`OPEN`인 채 오래된 건을 자동으로 닫지 않는다.**\
> 자동으로 닫으면 **아무도 안 본 것이 처리된 것으로 바뀐다** — DF6(삭제 금지)과 같은 은폐다.\
> 대신 `firstDetectedOn`으로 지속 기간이 드러나므로, **오래된 건이 목록 위로 올라오게** 하면 된다.

시간이 상태를 바꾸지는 않지만, 시간과 얽힌 값이 둘 있다.
...
| 버전 | 일자 | 내용 |
|---|---|---|
| v0.4 | 2026-09-16 | **가독성 재작성**(작성법 분리/복원/이연 — 내용 무변). 정향 문단·표/목록 lead-in 신설 · 설계 판단 blockquote에 한 문장 한 줄(`\`) 적용. 상태·전이·금지 ID·매트릭스·mermaid 전이도·ASCII 재발견 흐름 전부 보존 |
