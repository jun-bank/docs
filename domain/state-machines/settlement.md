...
> 이 문서를 다 읽으면 **왜 마감(`close`)과 집계(`calculate`)를 굳이 두 전이로 나눴는지(SF3 조용한 누락), 그리고 `FAILED`가 왜 종료 상태가 아닌지**를 설명할 수 있어야 한다.

정산은 집계라 재산출이 멱등이다.\
그 멱등성이 자동 재시도를 허용하는 근거이자, 이 상태 머신의 여러 결정을 관통하는 축이다.

---

## 1. 상태 목록

정산의 네 상태다 — 마감대기·산출중·완료·실패.
...
> **`FAILED`는 종료가 아니다.**\
> 매입 배치가 `INTERRUPTED`인 것과 같은 이유다 — 재시도하면 된다.\
> 다만 배치와 달리 **정산은 재산출이 멱등**(집계이므로)이라 재개 지점을 기억할 필요가 없다.

---

## 2. 전이도

마감·집계·실패·재시도·에스컬레이션·운영자 재개의 전이를 한 그림으로 보인다.
...
S2의 사후조건을 왜 이벤트 발행에서 끊었는지는 표 뒤에 따로 설명한다.

| # | 현재 | 전이 | 다음 | 조건 | 부수 효과 | 근거 |
|---|---|---|---|---|---|---|
| S1 | `PENDING` | `close` | `CALCULATING` | 마감 시각 도래 | **이후 거래는 다음 영업일 귀속** · **결제(C3)가 그 영업일 버킷을 닫고 합계를 전달** | BR-06·14·21 |
...
정산은 **집계**다.\
매입 반영처럼 상태를 누적하지 않으므로 몇 번을 계산해도 결과가 같다.\
**이것이 자동 재시도를 허용할 수 있는 근거**이며, 매입(BR-23)이 파일·레코드 멱등을 따로 둬야 했던 것과 대비된다.

> ★ **그래서 S2의 사후조건을 이벤트 발행에서 끊었다.**\
> 전표 기표와 `markSettled`는 재실행하면 중복되므로, 한 전이의 부수 효과로 묶으면 **재시도가 곧 이중 기표**가 된다.\
> BR-40이 이미 "원장 기표는 자금 이동과 별도 논리 단위"라고 요구한 분리이며, 이벤트를 받는 쪽이 **각자의 멱등**을 갖는다.

무엇이 멱등이고 무엇이 아닌지, 아닌 것은 어디서 멱등을 보장하는지 나눠 보면 이렇다.
...
막아야 하는 열한 시도다.\
표 뒤에 SF6이 막는 것이 "자동" 재시도뿐임을, SF3이 왜 가장 조용한 실패인지 설명한다.
...
> ★ **SF6이 막는 것은 "자동" 재시도뿐이다.**\
> 임계를 넘으면 자동 경로는 닫히지만 **운영자 강제 재개(S6)는 열려 있다.**\
> 이 구분이 없으면 `FAILED`가 사실상 종료 상태가 되어, 그날 순액이 영영 확정되지 않고 승인들이 `CAPTURED`에 잔류하며 대사 트리거(BR-42)까지 막힌다 — **문서가 "FAILED는 종료가 아니다"라고 선언한 것이 거짓이 된다.**

> **SF3이 가장 조용한 실패다.**\
> 마감 없이 집계해도 **에러가 나지 않는다** — 그냥 숫자가 작게 나올 뿐이다.\
> "에러 없이 돌았다 ≠ 완료"의 전형이며, 그래서 `close`를 별도 전이로 뒀다.\
> 마감과 집계를 한 조작으로 합치면 이 금지를 표현할 자리가 없어진다.

---

## 5. 전이 매트릭스

여섯 조작과 네 상태의 모든 조합을 채운다.
...
칸을 세면 **O 3 · O/X 3 · X 17 · ◎ 1 = 24칸**이다.

> **`FAILED` 행에 나가는 길이 두 개다.**\
> `retry`는 임계까지의 **자동** 경로, `resumeByOperator`는 임계 이후의 **수동** 경로.\
> 후자가 없으면 `FAILED`가 사실상 종료 상태가 된다(SF6 주석).
>
> **`FAILED` 행의 세 칸이 `retryCount` 임계 하나로 갈린다.**\
> `retry`는 임계 **미만**에서만, `escalate`·`resumeByOperator`는 임계 **이상**에서만 열린다.\
> 겹치면 운영자가 임계 전에 카운트를 0으로 밀어 SF6을 우회하거나(SF10), 정상 재시도 중인 건이 운영자 목록에 올라 진짜 미결이 묻힌다(SF11).
...
시간이 일으키는 네 전이다 — 마감·즉시 집계·재시도·진척 없음 감시.
...
### 마감이 곧 버킷을 닫는 것이다 (BR-21)

집계 주체는 정산이 아니라 결제(C3)다.\
아래가 그 역할 분담이다.

```
C3 결제 : 매입·환불 이벤트를 영업일 버킷에 누적
          마감(S1) → 그 버킷을 닫고 합계 2개를 C4에 전달
C4 정산 : 받은 합계로 순액만 계산 — 개별 매입을 읽지 않는다
```

> **정산이 직접 조회하면 두 가지가 깨진다.**\
> ① 애그리게이트 경계 근거(*"정산은 합계만 안다"*)가 무너지고 ② **마감 시점의 스냅샷을 어떻게 보장하나**가 새 문제가 된다 — 조회하는 동안 매입이 더 들어오면 SF3과 같은 조용한 누락이다.

### 빠뜨리기 쉬운 것

시간과 영업일 경계에서 놓치기 쉬운 다섯 가지를 정리한다.
...
아직 열린 세 의문(STS1·STS2·STS5)과 이미 닫힌 한 건(STS4)이다.
...
| ~~STS4~~ | ✅ **닫힘 (2026-09-15 = 애그리게이트 ST3): 독립 진행 — 다음 영업일 정산은 앞 날 `FAILED`와 무관하게 시작한다.** 순액 영업일 독립(각각 산출·합치지 않음 — §7)이라 막을 SF 전이가 없다. 앞 날 실패가 다음 날을 막으면 연쇄 정지. 미결은 개별 `resumeByOperator`로 해소하되 **누적 상한은 BR-42 강제실행 임계와 한 사슬**(수치 미확정) | 운영 정책 |
...
| 버전 | 일자 | 내용 |
|---|---|---|
| v0.7 | 2026-09-16 | **가독성 재작성**(작성법 분리/복원/이연 — 내용 무변) |
| v0.6 | 2026-09-15 | **STS4 닫힘 = ST3**(독립 진행 + 누적 상한). 애그리게이트 `settlement.md` v1.9와 동기. 막는 SF 전이 없음이 이미 정합 |
| v0.5 | 2026-08-04 | **STS3 확정 — 집계 주체는 결제(C3)**(BR-21). S1의 부수 효과에 버킷 닫기를 명시하고, 정산이 직접 조회하면 마감 스냅샷 보장이 새 문제가 되는 이유를 §7에 기록 |
