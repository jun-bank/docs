...
| **S6** | `FAILED` | **`resumeByOperator`** | `CALCULATING` | 운영자 식별자 + **사유 기록** | `retryCount = 0` | BR-37 |
...
| **`CALCULATING`** | ★ **마지막 진척 이후 경과 시간 초과** | **`FAILED`** | 감시 배치 | 미확정 (STS2) |
...
| **`CALCULATING`에 갇히는 경우** | 배치가 죽으면 `fail()`을 호출할 주체가 없다. **위 감시 전이가 이를 막는다** — 판정 기준은 매입 배치와 같이 "시작 후 경과"가 아니라 **"마지막 진척 이후 경과"** 다. 임계값만 STS2로 남는다 |
...
| **STS2** | `CALCULATING` 진척 없음 **판정 시간** (전이 자체는 §7에 등재됨) | 미확정 수치 |
