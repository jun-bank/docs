...
| **CF8** | `SUSPENDED` | `suspend` | ★ 이미 정지됨 — **명시 거절**(2026-08-05 사용자 판정: 멱등 무시가 아니라 상태를 정확히 알린다. UC4-1이 발견한 미판정 칸) | 재신고가 무음 통과하면 신고자가 상태를 오인한다 | `ALREADY_SUSPENDED` |
...
| **`SUSPENDED`** | ◎ | **O** (C2) | **O** (C3) | **O** (C4) | X (CF4) | **O/◎/X** (C6 / 기준일 / CF7) |
| **`TERMINATED`** | X (CF2) | X (CF1) | ◎ (CF8) | X (CF5) | X (CF3) | **O/◎/X** (C6 / 기준일 / CF7) |

**O 6 · O/X 1 · O/◎/X 3 · X 5 · ◎ 3 = 18칸**
...
| 버전 | 일자 | 내용 |
|---|---|---|
| v0.9 | 2026-08-05 | **CF8 신설** — `SUSPENDED × suspend` 미판정 칸(파일럿 UC4-1 발견)을 **명시 거절**로 확정(사용자 판정). 금지 8 → 9 |
