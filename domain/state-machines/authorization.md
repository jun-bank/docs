...
|---|---|---|---|---|---|---|
| T1 | `REQUESTED` | `authorize` | `AUTHORIZED` | 카드·한도·가용잔액 전부 통과 · **취소 예약 없음** | 홀딩 생성 · 승인번호 발번 · **카드 한도 + 계좌 한도** 사용액 증가 (BR-44 — 두 층 모두) | BR-04·05·15·17 |
| T2 | `REQUESTED` | `decline` | `DECLINED` | 검증 중 하나라도 실패 | **거절 사유 기록** | BR-15·36 |
| T3 | `REQUESTED` | **`createVoidedByTombstone`** | `VOIDED` | **취소 예약(tombstone) 존재** | **홀딩을 만들지 않고 한도도 쓰지 않는다** · 예약 소비 | BR-22 |
...
| T10 | `CAPTURED` | `refund`(전액) | `REFUNDED` | INV-1 · **INV-7 (`환불 누계 ≤ 매입액 − 발생 미수액`)** | **역분개** · 자금 반환 · **대응 미수 소멸** · 한도 복원 **없음** | BR-24·26·**43 ①** |
...
| T14 | 모든 상태 | `freeze`(종료 아닐 때) / `unfreeze`(**전 상태**) | **상태 불변** | 운영자 조작 | **자동 만료**(BR-28)·**매입 반영**(BR-50)에서 제외. **정산은 막지 않는다** | BR-28·50 |
...
| **F8** | 모든 상태 | `authorize` · `createVoidedByTombstone` | 둘 다 **새 애그리게이트 생성** 조작이다. 기존 건에 다시 적용할 수 없다 | 한 상관 식별자에 승인이 둘 | `DUPLICATE_AUTHORIZATION` (실제로는 멱등 레코드가 선차단 — BR-02) |
| **F9** | 종료 상태 (`DECLINED`·`VOIDED`·`REFUNDED`·`SETTLED`) | `freeze` | 조사할 진행 중 처리가 없다 | 운영자 목록에 죽은 건이 쌓인다 | `AUTH_TERMINAL` |
| **F19** | `REQUESTED` 외 모든 상태 | `decline` | 거절은 **검증 단계의 결론**이다. 성립한 뒤에는 거절이 아니라 취소다 | 성립한 승인이 거절로 뒤집혀 매입사와 상태가 갈린다 | `AUTH_ALREADY_DECIDED` |
| **F20** | `AUTHORIZED` · `EXPIRED` · 종료 상태 | `markSettled` | 정산은 **매입된 건만** 대상이다 (BR-21 — 순액 = 매입 − 취소) | 매입되지 않은 금액이 순액에 들어가 **매입사에 과다 지급** | `AUTH_NOT_CAPTURED` |
| **F21** | `CAPTURED` · 종료 상태 | `expire` | 홀딩이 이미 없다 | 없는 홀딩을 해제해 **다른 승인의 점유가 깎인다** (T9 함정과 같은 기전) | `AUTH_NO_HOLDING` |

> ⚠️ **F9와 `unfreeze`는 비대칭이다.** `freeze`는 종료 상태에서 금지지만 **`unfreeze`는 전 상태에서 허용**한다. 보류 중인 `AUTHORIZED` 건에 망취소가 오면 `reverse`는 막히지 않으므로(보류는 `expire`·`capture`만 막는다) **`frozen = true`인 채로 종료 상태에 도달**한다. 이때 해제 경로가 없으면 F9가 막으려던 상황이 **뒷문으로 그대로 발생**한다.
...
| **F17** | **`frozen = true`인 모든 상태** | `capture` | 조사 중 출금은 되돌리기 어렵다 (BR-50) | 부정거래 조사 중 자금 유출 | 격리 → **불일치 아님.** 별도 재처리 배치가 멱등 키를 승계 |
| **F18** | `CAPTURED` | `refund` (**상한 초과**) | `환불 누계 > 매입액 − 발생 미수액` (INV-7 · BR-43 ①) | ★ **받지 않은 미수분까지 반환**된다. 게다가 미수는 계좌에 남아 이후 입금을 상계(BR-34)해 **같은 금액을 두 번 회수** | 격리 → **M5 취소 불일치** |

> **F13이 가장 위험하다.** 우리가 거절한 거래에서 출금이 일어나면 고객은 승인한 적 없는 돈을 잃는다. 그런데 이건 **우리 코드 버그가 아니라 매입사가 잘못 보낸 파일**로 발생한다 — 그래서 **격리가 자동 방어선**이어야 한다.
>
> **F18은 상태가 아니라 금액이 만드는 금지다.** 상태는 `CAPTURED`로 정상이고 INV-1(누적취소액 ≤ 승인액)도 통과한다. **잔액 부족으로 미수가 생겼다는 사실**만이 이 금지를 발동시키며, 그래서 승인이 `receivableAmount`를 보유해야 한다.
>
...
| 현재 \ 조작 | `authorize` | `decline` | `reverse` | `void` | `expire` | `capture` | `refund` | `markSettled` | `freeze` | `unfreeze` |
|---|---|---|---|---|---|---|---|---|---|---|
| **`REQUESTED`** | O (T1) | O (T2) | - | - | - | - | - | - | - | - |
| **`AUTHORIZED`** | X (F8) | X (F19) | **O** (T4) | **O** (T5·T6) | **O** (T7) | **O/X** (T8 / F15) | ▲ (F7) | X (F20) | O (T14) | O (T14) |
| **`EXPIRED`** | X (F8) | X (F19) | **O** (T13) | **O** (T13) | ◎ | **O/X** (T9 / F15) | ▲ (F7) | X (F20) | O (T14) | O (T14) |
| **`CAPTURED`** | X (F8) | X (F19) | X (F1) | ▲ (F6) | X (F21) | X (F10) | **O/X** (T10·T11 / F18) | **O** (T12) | O (T14) | O (T14) |
| **`DECLINED`** | X (F8) | ◎ | **◎** | X (F5) | - | X (F13) | X (F7) | X (F20) | X (F9) | O (T14) |
| **`VOIDED`** | X (F8) | X (F19) | ◎ | X (F3) | X (F21) | X (F12) | X (F7) | X (F20) | X (F9) | O (T14) |
| **`REFUNDED`** | X (F8) | X (F19) | X (F4) | X (F4) | X (F21) | X (F14) | X (F7) | X (F20) | X (F9) | O (T14) |
| **`SETTLED`** | X (F8) | X (F19) | X (F2) | X (F2) | X (F21) | X (F11) | ▲ (F16) | ◎ | X (F9) | O (T14) |
...
- **`O` 18 · `O/X` 3 · `X` 41 · `◎` 5 · `▲` 4 · `-` 9 = 80칸.** 금지가 허용의 **2배**인 것이 정상이다 — 결제 도메인은 "무엇을 못 하는가"가 안전을 만든다.
...
- **`unfreeze` 열만 전 상태 `O`다.** 보류 목록에서 내리는 것은 언제나 가능해야 한다.
...
- **재처리는 원 배치를 다시 열지 않는다.** 별도 재처리 배치가 원 `(파일ID, 레코드ID)`를 **승계**한다 — 승계하지 않으면 두 번 지시하거나 파일이 재전송될 때 BR-23 멱등이 적용되지 않아 이중 출금이 된다.
