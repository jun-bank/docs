...
> 이 문서를 다 읽으면 **매입 배치가 어떤 상태를 거쳐 처리·중단·재개·완료되는지, 그리고 왜 건수 판정과 격리가 조용한 절단을 막는지**를 설명할 수 있어야 한다.

> **중단과 재개가 정상 경로다.**\
> 이 상태 머신은 "실패하지 않는 배치"가 아니라 **"실패해도 정확한 배치"** 를 표현한다.
...
> **`FAILED`가 없다.**\
> 배치는 실패하는 것이 아니라 **중단**된다 — 파일은 그대로 있고 진행분도 남아 있으므로, 재개하면 이어서 처리된다.\
> `FAILED`를 두면 "다시 못 하는 상태"라는 잘못된 신호를 준다.
...
아래 표가 매입 배치의 허용 전이 아홉 개다 — 수신부터 완료·격리 승격·재분류까지.
...
> ★ **B8의 채널 주석** (2026-08-06 CDS3 소급 — 사용자 확정): **이미 승격된 레코드의 운영자 API 재지시 = 명시 거절 `ALREADY_PROMOTED`.**\
> BF8의 무음 건너뜀은 **배치 재개 전용**이다 — 같은 판정을 채널이 가른다(재개 = 진행의 정상 경로 / API 재지시 = 운영자의 상태 오인).

> **B7의 조건이 이 문서에서 가장 중요하다.**\
> 건수로 판정하지 않으면 **조용한 절단**을 못 잡는다.\
> 파일 파서가 뒤쪽 1000건을 조용히 버려도 "에러 없이 돌았다"가 되고, 대사에서 M1(미매입) 1000건으로 뒤늦게 나타난다.

---

## 4. 금지된 전이 ★★

아래 표가 매입 배치가 막는 전이 열두 개다 — 건수 위장(BF1)과 격리 중복(BF8)이 핵심이다.
...
> ★ **격리 쪽 중복 방지가 INV-3의 전제다.**\
> 처리 집합만 막고 격리를 열어 두면 같은 레코드를 여러 번 격리해 **`|처리| + |격리|`를 인위적으로 채울 수 있고**, 그러면 파일 뒤쪽을 통째로 못 읽어도 완료 판정이 통과한다.\
> BF1이 막으려던 실패가 **다른 문으로 그대로 들어온다.**
...
아래 매트릭스가 네 상태 × 여덟 조작의 판정을 모은 정본이다.
...
> ★ **승격이 곧 멱등이다.**\
> "같은 집합을 공유한다"고만 정하면 **모순이 생긴다** — 그 레코드는 이미 `isolatedRecords`에 있어 재개 규칙에 따라 재처리 배치도 건너뛰게 된다.\
> 반대로 별도 집합을 쓰면 재처리 배치 둘 사이의 중복을 못 막는다.
>
> **격리에 있다 = 미반영 · 처리에 있다 = 반영됨.**\
> ③이 그 경계를 원자적으로 넘기므로 두 번 지시해도, 매입사가 파일을 재전송해도 ①에서 걸러진다.\
> 합계가 안 바뀌어 INV-3도 그대로다.
>
> **B8이 `COMPLETED`에서 허용되는 유일한 조작이다.**\
> 레코드 **추가**가 아니라 **이동**이므로 INV-4를 깨지 않는다.
>
> ⚠️ **그래서 사유 제한이 필수다**(INV-5 · BF12).\
> `COMPLETED`에서 유일하게 열린 문이므로, 사유를 안 가리면 **미승인(M2)·초과(M4)·후속(M7)·무효(M8) 격리까지 "처리됨"으로 승격**된다.\
> 반영되지 않은 레코드가 반영된 것으로 표시되고 건수 판정(INV-3)은 그대로 통과한다 — 이 문서가 방어선으로 삼은 **조용한 실패의 새 창구**다.

---

## 6. 동시성

아래 표가 매입 배치에서 경합이 생기는 상황과 그 처리다.

| 상황 | 처리 |
|---|---|
| **같은 파일을 두 배치가 동시에 처리** | ★ **`(발신 기관, fileId)` 유니크 제약**(R-06 — 제약의 열은 **둘**이다. 기관을 빼면 제약이 남의 키 공간까지 막아 다른 매입사의 파일이 통째로 누락된다) + 배치 단위 락. 애플리케이션 로직만으로는 못 막는다 |
| **같은 레코드 재처리** | ★ **`(발신 기관, fileId, recordId)`** 멱등 — 집합에 있으면 건너뛴다 (BF8 · R-06) |
...
| **재처리 배치와 원 파일 재전송의 경합** | 둘 다 원 ★ **`(발신 기관, fileId, recordId)`** 멱등 집합을 보므로 **먼저 커밋한 쪽만 반영**된다 |

---

## 7. 시간 기반 전이 ★

아래 표가 시간이 일으키는 전이 하나다 — 이게 없으면 `PROCESSING`에 영원히 갇힌다.
...
> 배치 프로세스가 그냥 죽으면 `interrupt()`를 호출할 주체가 없다.\
> 상태는 `PROCESSING`인데 아무도 처리하지 않는 상태가 되고, **재개도 안 되고 완료도 안 된다.**\
> 그러면 그날 매입 전체가 대사에서 M1(미매입)으로 잡힌다.
>
> **진척 판정은 "시작 후 경과 시간"이 아니라 "마지막 레코드 커밋 이후 경과 시간"이어야 한다.**\
> 전자로 하면 정상적으로 오래 걸리는 대용량 배치를 중단시킨다.

아래 표가 이 감시 전이에서 놓치기 쉬운 두 항목이다.
...
아직 확정되지 않은 물음(닫힌 BFS1 포함)을 남긴다.

### 열린 의문

| # | 의문 | 영향 |
|---|---|---|
| ~~BFS1~~ | ✅ **확정 (2026-08-06 계약 전수 B-8)**: `totalRecords` 미독 파일은 **`RECEIVED`에서 거부하고 매입사에 재전송을 요청**한다 — 부분 처리하면 INV-3이 성립하지 않아 `COMPLETED`에 영영 도달하지 못한다(UC-10 §7 계약과 동기) | INV-3 · 애그리게이트 CB2 |
...
| 버전 | 일자 | 내용 |
|---|---|---|
| v1.0 | 2026-09-16 | **가독성 재작성**(작성법 분리/복원/이연 — 내용 무변). 정향 문단·표/목록 lead-in 신설 · ⚠️·★ 설계 판단 blockquote에 한 문장 한 줄(`\`) 적용. 상태·전이·금지 ID·매트릭스·mermaid 전이도·재처리 배치 ASCII 전부 보존 |
| v0.9 | 2026-08-06 | **R-06 기관 축 전파 일소**: §6 동시성 3행 — 파일 유니크 제약 `(발신 기관, fileId)` · 레코드 멱등 `(발신 기관, fileId, recordId)`(재처리·재전송 경합 포함) |
