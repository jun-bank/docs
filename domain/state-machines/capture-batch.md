...
> ★ **B8의 채널 주석** (2026-08-06 CDS3 소급 — 사용자 확정): **이미 승격된 레코드의 운영자 API 재지시 = 명시 거절 `ALREADY_PROMOTED`.** BF8의 무음 건너뜀은 **배치 재개 전용**이다 — 같은 판정을 채널이 가른다(재개 = 진행의 정상 경로 / API 재지시 = 운영자의 상태 오인).
