...
> 이 문서를 다 읽으면 **왜 미수 상태가 둘뿐인데도 이 문서의 값어치가 §4 금지 전이에 있는지, 특히 종결된 채권을 다시 회수하는 VF1을 왜 막아야 하는지**를 설명할 수 있어야 한다.

상태가 둘뿐이라 이 문서의 값어치는 §4 금지 전이에 있다.\
특히 **VF1(종결된 채권의 재회수)** — 4차 리뷰가 치명으로 잡은 결함이며, 이전 구조에서는 그것을 막을 자리가 **어디에도 없었다.**

---

## 1. 상태 목록

미수 채권의 세 상태다 — 없음·미결·종결.
...
> **보류(`frozen`)는 상태가 아니라 직교 플래그다.**\
> `recover` **하나만** 막는다 — 상태로 올리면 2종이 4종이 되고 얻는 것이 없다 (README ⑥).

> ⚠️ **`CLOSED`에 삭제·정리 전이가 없다.**\
> 미수는 채권 이력이므로 종결 후에도 남는다.\
> 영원히 회수되지 않는 채권의 처리(대손)는 현 범위 밖 → **RCS2**.

---

## 2. 전이도

발생·회수·소멸의 전이를 한 그림으로 보인다.
...
여섯 전이의 조건·부수 효과·근거를 정리한다.\
V1의 전표 분기와 `recover`·`writeOff`의 차이는 표 뒤에 따로 설명한다.

| # | 현재 | 전이 | 다음 | 조건 | 부수 효과 | 근거 |
|---|---|---|---|---|---|---|
| V1 | `ABSENT` | `incur` | `OPEN` | `amount > 0` · 같은 `(origin, sourceRef)` 없음 (INV-6) | 채권 생성 · `origin`·`sourceRef` 확정 · `incurredBusinessDate` 확정(FIFO 키) · 전표는 **`origin`별로 갈린다**(아래 산문) | BR-20·38 |
...
V1의 전표는 **`origin`별로 갈린다** — `CAPTURE`이면 대) 매입사 미지급금, `DEPOSIT_REVERSAL`이면 대) 예치금이다.

> ⚠️ **부수 효과·조건 열에 `〃`를 쓰지 않는다** (애그리게이트 README).\
> 위 표에서 전부 풀어 썼다 — 이 프로젝트에서 `〃`가 **네 라운드 연속으로 결함을 만들었다.**

V2·V3과 V4·V5는 같은 조작인데 조건이 다르다.\
아래 표가 `recover`와 `writeOff`의 차이다.
...
> **`writeOff`에 보류를 걸면 안 된다.**\
> 조사가 끝날 때까지 **이미 없어진 채무가 장부에 남는다.**
...
막아야 하는 아홉 시도다.\
표 뒤에 VF1이 왜 이 문서의 존재 이유인지, VF3·VF8·VF9가 왜 오류가 아닌지 설명한다.

| # | 현재 | 시도 | 왜 금지인가 | 위반 시 | 응답 |
|---|---|---|---|---|---|
| **VF1** | `CLOSED` | `recover` | ★ **종결된 채권은 회수할 것이 없다** (INV-4) | **없는 채권을 회수한다** — 고객이 낸 돈이 채권으로 흡수되어 잔액이 늘지 않는다 | `RECEIVABLE_CLOSED` |
...
| **VF9** | `OPEN` · `CLOSED` | `writeOff` (**`amount = 0`**) | 소멸할 것이 없다 | — | **호출부가 걸러야 한다**(아래 산문) |

VF1의 위반 결과가 4차 리뷰의 치명이었다.\
이전 구조에서는 막을 자리가 없어, 정산 완료된 거래의 미수가 계속 회수됐다.

VF9는 오류 응답이 아니라 호출부의 책임이다.\
승인 `refund()`는 *소멸액 > 0 이고 미수가 미결일 때만* 호출한다.\
조건 없이 부르면 두 번째 환불에서 `CLOSED` 미수에 `writeOff(0)`이 가 **VF2로 정상 환불이 실패**한다.

> ★ **VF1이 이 문서의 존재 이유다.**\
> 이전 구조에서는 미수가 계좌 합계였고 회수 진행도가 승인에 있어서, *"이 채권은 이미 끝났다"* 를 판정할 주체가 없었다.\
> 그래서 **정산 완료된 거래의 미수가 계속 회수됐다**(4차 Q3 — 치명).\
> 지금은 **한 애그리게이트가 자기 종결을 안다.**
>
> **VF3만 오류가 아니다.**\
> 보류는 정상 상태이고, 회수 **대상 선정 단계에서 걸러진다**(BR-34).\
> 오류로 만들면 정상 입금 처리가 실패한다.
>
> **VF8은 조작 부재로 강제된다.**\
> `amount`를 바꾸거나 채권을 지우는 메서드가 **아예 없다.**\
> 규칙을 문서에만 적으면 언젠가 누가 추가하지만, 조작이 없으면 추가하려는 순간 리뷰에 걸린다.

---

## 5. 전이 매트릭스

다섯 조작과 세 상태의 모든 조합을 채운다.
...
칸을 세면 **O 4 · O/X 2 · X 5 · - 4 = 15칸**이다.

> ⚠️ **이 표는 `frozen`을 상태로 세지 않는다.**\
> 보류는 `recover` **한 칸**에만 영향을 주는 직교 플래그다(VF3).\
> 상태로 올리면 3행이 6행이 되고 다섯 칸은 그대로다.
>
> **`CLOSED` 행에서 `unfreeze`만 열려 있다.**\
> 보류는 `recover`만 막고 `writeOff`는 막지 않으므로, **보류 중인 미수가 환불로 종결될 수 있다.**\
> 그때 `frozen = true`인 채로 `CLOSED`에 도달하고, 해제 경로가 없으면 **보류 목록에서 내릴 방법이 없다.**\
> 승인의 `freeze`/`unfreeze` 비대칭(F9)과 같은 이유다.
>
> **`X`가 5개인데 그중 4개가 `CLOSED` 행이다.**\
> 이 상태 머신의 방어선은 **"끝난 채권은 아무도 못 건드린다"** 하나로 요약된다.
...
동시 상황별 처리를 정리한다.\
락 순서를 정하지 않으면 교착하므로, 그 순서 규칙을 표 뒤에 둔다.
...
> ⚠️ **E3와 E4가 미수에서 만난다.**\
> 락 순서를 정해 두지 않으면 교착한다 — **미수를 (발생 영업일, 식별자) 순으로 먼저 잠그고** 계좌·승인을 나중에 잡는다.\
...
**시간이 일으키는 상태 전이가 없다.**\
미수는 스스로 갚히지 않고, 오래됐다는 이유로 사라지지도 않는다.

> **오래된 미수를 자동으로 닫지 않는다.**\
> 자동으로 닫으면 **회수하지 못한 채권이 회수된 것처럼 보인다** — 불일치를 자동 정정하지 않는 것(BR-09)과 같은 이유다.\
> 대신 `incurredBusinessDate`로 **며칠째 미회수인지**가 드러난다.

시간과 관련은 있지만 상태를 바꾸지 않는 항목들이다.
...
| ~~RCS5~~ | ✅ **닫힘 (2026-09-15 = 애그리게이트 RV4): 회수(`recover`)만이 종결.** `DEPOSIT_REVERSAL` 미수는 고객 채무라 회수로만 `CLOSED`, `writeOff`(매입사 환불 전용)는 부적용이 정상. 자동 종결 금지(§7)와 정합. 회수 외 소멸 = 대손(RCS2)이며 범위 밖과 함께 판정 | BR-38 · RCS2 |
...
| v0.4 | 2026-09-15 | **RCS5 닫힘 = RV4**(회수만이 종결). 애그리게이트 `receivable.md` v0.5와 동기 |
