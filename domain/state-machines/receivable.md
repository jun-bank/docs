...
| V3 | `OPEN` | `recover` (**잔여 0이 됨**) | `CLOSED` | 〃 | `recoveredAmount` 증가 · 계좌의 `receivableBlockLifted` 해제 판단 | BR-34·45 |
| V4 | `OPEN` | `writeOff` (**잔여 > 0 유지**) | `OPEN` | `amount ≤ outstanding()`. **`frozen`과 무관** | `writtenOffAmount` 증가 · 전표 `차) 매입사 미지급금 / 대) 미수금`. **자금 이동 없음** | BR-43 ① |
| V5 | `OPEN` | `writeOff` (**잔여 0이 됨**) | `CLOSED` | 〃 | 〃 + 차단 해제 판단 | BR-43 ①·45 |
...
| **VF6** | `OPEN` · `CLOSED` | `incur` (**같은 승인**) | 한 매입의 부족분은 **채권 한 건**이다 | 같은 부족분이 두 채권이 되어 **이중 청구** | `RECEIVABLE_ALREADY_EXISTS` |
...
|---|---|---|---|---|---|
| **`ABSENT`** | **O** (V1) | - | - | - | - |
| **`OPEN`** | X (VF6) | **O/X** (V2·V3 / VF3·VF4) | **O/X** (V4·V5 / VF5) | **O** (V6) | **O** (V6) |
| **`CLOSED`** | X (VF6) | X (VF1) | X (VF2) | X (VF7) | **O** (V6) |
...
| **여러 미수에 걸친 배분 중 일부 실패** | E3가 한 커밋이므로 **전부 반영되거나 전부 롤백.** 부분 배분이 남지 않는다 (계좌 INV-7) |
| **보류와 회수의 경합** | 보류가 미수 락을 잡는다. **회수가 이겼다면 그 회수는 유효** — 보류 이전에 들어온 자금이므로 정상이다 |
| **환불과 회수의 동시 도착** | 환불은 승인+미수(E4), 회수는 계좌+미수들(E3). **미수가 양쪽의 교집합**이므로 락이 직렬화한다 |
