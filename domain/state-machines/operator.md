...
| **O3** | `SUSPENDED` | `resume` | `ACTIVE` | 주체 = **책임자 + 대상 포함 스코프**(BR-55) | ★ **권한은 자동으로 부활하지 않는다 — 재인증 게이트**(해제 시 기존 grant는 `lastResumedAt` 이후의 `GrantRecertified`가 있어야 유효 — 2026-08-06 확정 · BR-59 ③) · **`lastResumedAt` 기록** · **`authzVersion` 증가** | 애그리게이트 §5 · D-9 |
...
>
> ⚠️ **O2(정지)와 O4(해지)의 처리가 다르다 — 그러나 "해제하면 권한이 돌아온다"는 뜻은 아니다.** 정지는 **계정 상태가 가역**이고(O3으로 돌아온다) 해지는 불가역이다. 다만 **권한 쪽은 둘 다 자동 복구가 없다** — 정지 해제는 **재인증 게이트**(recertify 전 무효), 해지는 회수 그 자체다(대장 D-9의 구현 확정 2026-08-06 · BR-59 ③).
>
