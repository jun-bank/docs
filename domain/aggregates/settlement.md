...
| **정본** | ⚠️ **미정** — *"C3 결제의 영업일 버킷"* 이라고 적었으나 **그런 버킷이 모델에 없다** (DC-002) |
| **트랜잭션 경계** | **마감(`close`) 시점의 스냅샷**이고 이후 `status = 완료`에서 불변(INV-4). 어긋나는 구간은 마감 커밋 안뿐이다 |
| **탐지** | ⚠️ **불충분** — BR-30 외부 대사는 매입사 파일과 비교할 뿐, `refundTotal`의 **내부 구성 합을 독립적으로 재현하지 않는다** (DC-002) |

> ⚠️ **이 ③은 아직 성립하지 않는다.** 양식이 요구하는 세 칸 중 **정본과 탐지가 비어 있다** —
> 존재하지 않는 버킷을 정본으로 삼을 수 없고, 외부 대사는 내부 구성 합의 탐지 장치가 아니다.
> **DC-002가 닫을 때까지 미완으로 표시한다** (`state-machines/README.md` ③ 체크리스트와 같은 판정).
>
...
| **합계 필드는 없는가** | `captureTotal`·`refundTotal`이 해당 — ③ 등재했으나 **정본·탐지 미완**(§2 · DC-002) |
