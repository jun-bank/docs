...
| | 내용 |
|---|---|
| **등식** | `captureTotal = Σ(그 영업일 반영 매입액)` · `refundTotal = Σ(그 영업일 확정 환불액)` — **정본은 C3 결제의 영업일 버킷** (BR-21) |
| **트랜잭션 경계** | **마감(`close`) 시점의 스냅샷**이고 이후 `status = 완료`에서 불변(INV-4). 어긋나는 구간은 마감 커밋 안뿐이다 |
| **탐지** | **BR-30 외부 대사**가 3자 비교로 매입 합계를 검증한다 |
...
| **합계 필드는 없는가** | `captureTotal`·`refundTotal`이 해당 — **의식적 예외 ③** 로 등재(§2) |
