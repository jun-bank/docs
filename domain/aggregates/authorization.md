...
| **`recoveredAmount`** | `Money` | **이 승인으로 실제 회수한 총액.** 매입 시 `withdrawnAmount`로 시작해 **미수 상환 배분 시 증가**(BR-34), **반환 시 감소**한다 (BR-43 ①) |
...
| **INV-8** | `withdrawnAmount`는 **불변**이고 `refundedTotal`은 **단조 증가**한다. `recoveredAmount`는 상환 배분으로 늘고 반환으로 준다 | BR-43 ① | 상한이 스스로 커지거나 회수 이력이 사라진다 | `capture()` 후 · `refund()` 후 |
...
| **INV-10** | **`recoveredAmount ≤ 잔여 채무`** (= `capturedAmount − refundedTotal`) | BR-43 ① | 회수액이 채무를 넘은 채 남으면 **돌려줘야 할 돈을 안 돌려준 상태**다 | `refund()` 후 |
...
| **환불** | `refund(amount)` | 상태 = 매입됨, **INV-7** (`refundedTotal + amount ≤ capturedAmount`) | `refundedTotal` 증가 → **반환액 파생 계산** → `recoveredAmount` 감소. 잔여 채무 0이면 상태 = 환불됨. `cancelledAmount`는 건드리지 않는다 (INV-9) | `Refunded` |
| **미수 상환 배분** | `recoverReceivable(amount)` | 상태 ∈ {매입됨, 정산완료}, `recoveredAmount + amount ≤ 잔여 채무` | `recoveredAmount` 증가 (BR-34 FIFO 배분 결과) | `ReceivableRecovered` |
...
| ~~AU10~~ | 이미 상계된 미수 뒤의 환불 | **`recoveredAmount` 추적 + FIFO 배분** → BR-34 확장 |
