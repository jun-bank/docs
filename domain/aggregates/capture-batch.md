...
| **INV-4** | `status = 완료됨` 이후 레코드가 **추가**되지 않는다. 두 집합 사이의 **승격**(격리 → 처리)은 합계를 바꾸지 않으므로 예외다 | — | 완료 후 변경 / 반대로 승격까지 막으면 **보류 격리를 영영 반영할 수 없다** |
...
| **격리 승격** | `promoteIsolated(recordId)` | 격리 목록에 있음. **`완료됨`에서도 허용** (INV-4 예외) | 격리 목록에서 제거 + 처리 집합에 추가. **합계 불변** | `IsolatedRecordPromoted` |
