...
| ★ **격리 재분류** | `reclassifyIsolated(recordId, at)` | 격리 목록에 있음 **AND 사유 = 보류 AND 원승인이 종료 상태**(`VOIDED`·`REFUNDED` 등 — 같은 컨텍스트 C3의 승인 종료 이벤트가 트리거). **시스템 조작**(BR-54 예외 ②) — 이미 재분류됐거나 대상이 없으면 ◎ 무시(이벤트 재전달의 정상 경로) | 격리 사유를 **M8로 교체**(이전 사유는 이력 append 보존) — 이후 승격은 BF12에 막힌다. 배치만 변경 — **불일치 적재는 Outbox**(isolate와 같은 경로) | `IsolatedRecordReclassified` |
...
| 버전 | 일자 | 내용 |
|---|---|---|
| v1.5 | 2026-08-06 | ★ **정본 판정 반영 (UC10-1·UC14-2 — 사용자 합의)** — 격리 사유 **기한 초과(M19)** 추가(BR-19) · `reclassifyIsolated(recordId, at)` 신설(보류 격리 중 원승인 종료 → M8 자동 재분류·이력 보존·불일치 적재) · INV-5에 M19 포함 |
