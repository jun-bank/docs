...
| # | 불변식 | 근거 | 위반 시 |
|---|---|---|---|
| **INV-1** | ★ **`(발신 기관, fileId)`는 유일하다** — 유일성의 스코프는 **발신 매입사**다(2026-08-06 개정 R-06: 파일 ID는 매입사가 자기 체계로 채번하므로 전역 유일이 아니다) | BR-23 | 같은 파일이 두 번 처리 / ★ 반대로 전역 유일을 가정하면 **다른 매입사의 같은 `fileId`가 중복으로 판정되어 파일 하나가 통째로 반영되지 않는다**(무음 누락) |
...
| **INV-4** | `status = 완료됨` 이후 레코드가 **추가**되지 않는다. 예외는 둘 — **보류 사유 격리의 승격**(격리 → 처리, 합계 불변) · ★ **격리 사유 재분류**(레코드 이동·추가가 아니라 **속성 교체** — 2026-08-06 B9. 배타 열거를 유지하기 위해 등재) | — | 완료 후 변경 / 반대로 승격까지 막으면 **보류 격리를 영영 반영할 수 없다** |
| **INV-5** | 승격 대상은 **보류 사유 격리뿐**이다 | BR-50 | ★ M2·M4·M7·M8·M19 격리가 승격되면 **반영되지 않은 레코드가 "처리됨"으로 표시**되고 건수 판정은 그대로 통과한다 — 조용한 실패의 새 창구 |
...
| ★ **격리 재분류** | `reclassifyIsolated(recordId, at)` | 격리 목록에 있음 **AND 사유 = 보류 AND 원승인이 종료 상태**(`VOIDED`·`REFUNDED` 등 — 같은 컨텍스트 C3의 승인 종료 이벤트가 트리거). ★ **대상 도출**: 종료 이벤트의 승인 ID → 격리 레코드(격리 레코드는 isolate 매칭 시점부터 **원승인 ID를 보유**한다 — 역인덱스 조회. 같은 승인의 보류 격리가 여러 배치에 있으면 각 배치에서 처리). **시스템 조작**(BR-54 예외 ②) — 이미 재분류됐거나 대상이 없으면 ◎ 무시. ★ **멱등 키가 따로 없다** — 사전조건(사유 = 보류) 검사 자체가 멱등 경계다(이벤트 재전달 안전 — **처리 실패도 재전달로 재시도**한다). 교체는 **배치 애그리게이트 락 아래 원자**다 | 격리 사유를 **M8로 교체**(이전 사유는 이력 append 보존 — `at` = 재분류 시각: 이력의 시각이자 **불일치 적재의 귀속 영업일**(BR-14) 산정 입력) — 이후 승격은 BF12에 막힌다. 배치만 변경 — **불일치 적재는 Outbox**(isolate와 같은 경로) | `IsolatedRecordReclassified` |
...
| ★ **기한 초과** (승인일 + 10일 CNP 경과) | **BR-19** | **M19 기한 초과 매입** (2026-08-06 UC10-1 확정 — 격리 즉시 적재) |
| **승인이 유효하지 않음** (`DECLINED`·`VOIDED`·`REFUNDED`) | **BR-51** | **M8 무효 승인 매입** |
| 이미 매입된 승인 | BR-32 | M7 후속 매입 |
| 금액 초과 | BR-16 ③ | M4 금액 초과 매입 |
| **원승인이 보류 중** (`frozen = true`) | **BR-50** | **없음 — 불일치가 아니다**. ★ 단 **보류 중 원승인이 종료되면 M8로 자동 재분류**된다(`reclassifyIsolated` — BR-50 · UC14-2 확정) |
