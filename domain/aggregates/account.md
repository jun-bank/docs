...
| **RC-2** | 회수액의 **미수별 배분 합계 = 회수액** | BR-34 | 배분이 새거나 부풀면 채권 총액이 실제 회수와 어긋난다 | E3 커밋 안에서 검사 + BR-41 |
...
| **매입 출금** | `capture(captureAmount, heldAmount)` | `heldAmount ≤ holdTotal` | `holdTotal` **전액 해제**, `balance` 감소. **부족분은 `Receivable.incur()`** — **같은 커밋**(E2). 갈리면 출금은 됐는데 채권이 없다 | `Withdrawn` |
...
| **합계 필드는 없는가** | `holdTotal`이 해당 — **의식적 예외 ③** 로 등재(§2), 등식은 **대사 불변식 RC-1**. `dailyUsage`는 계좌 자신의 누적이라 해당 없음 |
