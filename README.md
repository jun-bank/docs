...
| **5** | 샘플 구현 + 실험 38건 | ⏳ **진행 중** | 문서가 요구한 것이 실제로 성립하는지를 코드로 친다. 지금까지: ⑴ **인프라 배포 파이프라인**(infra repo · Go) — deploy-agent 게이트1(HMAC 서명·멱등) · 게이트2(OIDC claim 행렬 + **JWKS 실 페치**) · dispatch(pull·compose up·CD-1 헬스) · 배포 창 락(fencing·lease) · 모드(dev/운영 fail-closed) · 이력. S0·S2 마일스톤. ⑵ **워킹 스켈레톤**(현재 활성) — 최소 Kotlin/Spring 서비스 3 + Spring Cloud Gateway → docker compose 라우팅 실증 → 첫 실배포 예정 |
...
> **설계는 이 `docs` 저장소, 구현은 각 repo.** 이 문서가 설계 정본 허브이고, 실제 코드는 별도 저장소에 산다 — `infra`(Go 인프라 도구 deploy-agent·dispatcher·watchdog) · `common`(Kotlin 공통 라이브러리 — 계약·인프라만 공유, 도메인 비공유 ADR-025) · `core`·`settlement`·`ledger`(Kotlin/Spring 서비스 = 배포 3단위, 현재 워킹 스켈레톤) · `gateway`(내부 API 게이트웨이 Spring Cloud Gateway). "그 서비스를 어떻게 돌리나"는 각 repo README, "왜 그렇게 설계했나"는 여기.
...
> ★ **처음부터 전부 읽는 상세 순서는 [`READING-ORDER.md`](READING-ORDER.md)** — 일곱 층(제품→도메인→아키텍처→유스케이스→설계변경→배포)으로 무엇을 왜 그 순서로 읽는지 안내한다. 아래는 그 요약이다.
